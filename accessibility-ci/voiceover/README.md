...
The first ordinary-window run, [35295064401](https://github.com/ryparker/aloud/actions/runs/35295064401), verified permission, startup and cleanup but stopped before modal activation: native VoiceOver remained on the toolbar while DOM focus identified the opener. The next experiment explicitly selects web content using VoiceOver's Item Chooser before the existing assisted opener placement and sentinel. It adapts the native entry sequence from [Guidepup's browser helper](https://github.com/guidepup/guidepup-playwright/blob/main/src/voiceOverTest.ts) without adding a Playwright browser or dependency.

Browser entry is separate setup evidence with at most seven SDK calls, including eleven typed keystrokes, with one attempt per native action, and deadline checks around awaited work. A command exceeding the deadline fails the setup before another command starts. The chooser must be identified before typing, and its search output is retained. Echoed query text does not verify candidate selection; interaction must leave the toolbar and chooser, and the unchanged opener sentinel remains required. The owned front window, fixture URL, run token and foreground application are checked before and after. Every setup command remains recorded. Setup speech cannot satisfy the later opener or modal capture. Screenshots retain failed entry state, and DOM document focus is now recorded separately from its active element.

Run [35296060809](https://github.com/ryparker/aloud/actions/runs/35296060809) exposed the cost of `type()` with full capture: Guidepup performs one captured action per character, so the eleven-character query took about 82 seconds and exceeded the setup deadline. Typing now uses `capture: "initial"`, which samples once per character. Setup metadata records that distinct mode. Scenario captures retain their existing full polling policy.

A second setup observation showed the Item Chooser on the desktop while the command's stored phrase identified only its highlighted item. If the initial chooser caption is absent, setup now retains up to four passive native last-phrase reads at 200 ms intervals. Each native read has its own three-second timeout, remains inside the setup deadline checks, and cannot issue another reader command. Read failures or absent chooser identity still stop setup. This readiness evidence applies only before typing and cannot replace a required scenario capture.

Run [35296989526](https://github.com/ryparker/aloud/actions/runs/35296989526) completed native web-content entry in 23.34 and 19.23 seconds. Both interactions identified the fixture's `main` inside web content, but `document.hasFocus()` changed from true before entry to false afterward. Both original opener sentinels then reported the page's scroll area despite DOM `activeElement` identifying the opener. The modal was not activated.

The setup correction invokes the pinned `moveKeyboardFocusToCursor` command once after interacting with the page, before JavaScript opener placement. Apple documents [VO-Command-F4](https://support.apple.com/en-mide/guide/voiceover/cpvokys04/mac) as moving keyboard focus to the VoiceOver cursor. Setup then requires document focus as well as the existing window, URL, run-token and foreground checks. This transient focus handoff does not alter VoiceOver preferences or establish that the earlier runtime tracking settings were disabled. The original opener and modal captures remain required.

The handoff was exercised in [run 35297738621](https://github.com/ryparker/aloud/actions/runs/35297738621), tooling commit `49db1217a37e8c1650a8496644e2e340a1da83bb`. The corrected attempt completed the native focus command but remained on `main` with document focus false, so the new guard stopped it. The broken attempt stopped earlier after four native phrase reads failed to identify the chooser, although its screenshot showed the chooser open. Both readers stopped and their profile volumes unmounted; no scenario step or modal activation ran. All 111 tooling tests passed, but corrected/broken native calibration and the ten-run sample remain unvalidated.

The next investigation needs a minimal native control fixture that establishes a focusable page control and verifies chooser state independently of its changing last-phrase text. The observed handoff on `main` did not establish document focus. Neither this failure nor the chooser's missing title identifies a Safari, VoiceOver, or Guidepup root cause. Keep this workflow informational until the unchanged opener and modal evidence requirements can be satisfied. All hosted experiments recorded here have completed.
